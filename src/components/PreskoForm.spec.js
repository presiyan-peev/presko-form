...
describe("PreskoForm.vue", () => {
  describe("Basic Rendering", () => {
    it("renders specified components for fields", async () => {
      ...
      const actualFields = [
        {
          ...
          props: { label: "Name" },
          value: "DefaultNameInField",
        },
        {
          propertyName: "email",
          component: "StubAppInput",
          props: { label: "Email" },
          value: "default.email@example.com",
        },
      ];
      const actualModelValue = {};

      console.log(
        "[Test Part 1] Setting props with actualFields and actualModelValue:",
        JSON.stringify(actualFields),
        JSON.stringify(actualModelValue)
      );
      ...
      console.log(
        "[Test Part 1] Number of StubAppInput components found:",
  ...
  describe("v-model integration", () => {
    it("updates input components when modelValue prop changes", async () => {
      const fields = [
        { propertyName: "name", component: "StubAppInput", value: "Initial" },
      ];
      ...
      await wrapper.setProps({
        modelValue: { ...initialModelValue, name: newName },
      });
      await nextTick();
    ...
    it('emits "update:modelValue" with the correct data when an input changes', async () => {
      ...
      expect(emittedUpdateEvents[emittedUpdateEvents.length - 1][0]).toEqual(
        expectedEmittedData
      );
    });
  ...
  describe("Validation and Submission Flow", () => {
    ...
    it('emits "submit:reject" and displays errors when form is submitted with invalid data', async () => {
      ...
      expect(emailInput.props("errorMessages")).toBe(
    ...
    it('emits "submit" with form data when form is valid and submitted', async () => {
      ...
      await nextTick();
      await wrapper.setProps({
        modelValue: { name: "John Doe", email: "john.doe@example.com" },
      });
      await nextTick();
      await wrapper.find("form").trigger("submit.prevent");
      await nextTick();
      ...
      const fieldsWithCustomError = [
        {
          ...
          component: "StubAppInput",
          rules: [
            { name: "required", customErrorMsg: "Username must be provided." },
          ],
          props: { label: "Username" },
          value: "",
        },
      ];
      const customWrapper = createFormWrapper({
  ...
  describe("Sub-form Functionality", () => {
    ...
    const mountWithSubForm = async (modelValue = {}) => {
      const completeModelValue = {
        fullName: modelValue.fullName,
        address:
          typeof modelValue.address === "object" ? modelValue.address : {},
        ...modelValue,
      };
      const wrapper = createFormWrapper({
        fields: mainFieldsConfig,
        modelValue: completeModelValue,
      });
      await nextTick();
      await nextTick();
      return wrapper;
    ...
    it("renders nested PreskoForm components for sub-form configurations", async () => {
      ...
      // Debug: Log the wrapper HTML
      console.log("Wrapper HTML:", subFormWrapper.html());
      console.log("Fields prop:", subFormWrapper.props("fields"));
      console.log("ModelValue prop:", subFormWrapper.props("modelValue"));

      const allForms = subFormWrapper.findAllComponents(PreskoForm);
      console.log("Number of PreskoForm components found:", allForms.length);

      // Try finding by CSS class instead
      const formElements = subFormWrapper.findAll(".presko-form");
      console.log(
        "Number of .presko-form elements found:",
        formElements.length
      );

      expect(formElements.length).toBe(2);

      // Find the nested form inputs to verify they exist
      const stubInputs = subFormWrapper.findAllComponents(StubAppInput);
      console.log(
        "Number of StubAppInput components found:",
        stubInputs.length
      );
      expect(stubInputs.length).toBe(3); // fullName + street + city

      // Verify the inputs have the correct labels
      const labels = stubInputs.map((input) => input.props("label"));
      ...
      // Verify the inputs have the correct model values
      const streetInput = stubInputs.find(
        (input) => input.props("label") === "Street"
      );
      const cityInput = stubInputs.find(
        (input) => input.props("label") === "City"
      );
      expect(streetInput.props("modelValue")).toBe("DefStreet");
      expect(cityInput.props("modelValue")).toBe("DefCity");
    });
    ...
    it("binds v-model correctly for sub-forms (nested data structure)", async () => {
      ...
      console.log("Initial modelValue:", subFormWrapper.props("modelValue"));

      const streetInput = subFormWrapper
        .findAllComponents(StubAppInput)
        .find((c) => c.props("label") === "Street");

      console.log(
        "Street input initial value:",
        streetInput.props("modelValue")
      );
      ...
      console.log(
        "After street input change, modelValue:",
        subFormWrapper.props("modelValue")
      );
      console.log("All emitted events:", Object.keys(subFormWrapper.emitted()));

      const emittedUpdate = subFormWrapper.emitted("update:modelValue");
      console.log("update:modelValue events:", emittedUpdate);
    ...
    it("propagates modelValue changes from parent to sub-form inputs", async () => {
      subFormWrapper = await mountWithSubForm();
      await nextTick();
      await subFormWrapper.setProps({
        modelValue: {
          fullName: "Changed Name",
          address: { street: "Updated Street", city: "Updated City" },
        },
      });
      await nextTick();
      const nameInput = subFormWrapper
        .findAllComponents(StubAppInput)
        .find((c) => c.props("label") === "Full Name");
      const streetInput = subFormWrapper
        .findAllComponents(StubAppInput)
        .find((c) => c.props("label") === "Street");
      const cityInput = subFormWrapper
        .findAllComponents(StubAppInput)
        .find((c) => c.props("label") === "City");
    ...
    it("validates sub-forms and rejects submission if sub-form is invalid", async () => {
      ...
      const streetInputInSubForm = subFormWrapper
        .findAllComponents(StubAppInput)
        .find((c) => c.props("label") === "Street");
      expect(streetInputInSubForm.props("error")).toBe(true);
      expect(streetInputInSubForm.props("errorMessages")).toBe(
    ...
    it("emits submit with nested data when main and sub-forms are valid", async () => {
      ...
      expect(subFormWrapper.emitted("submit")[0][0]).toEqual({
        fullName: "Another Valid Name",
        address: { street: "789 Good St", city: "Validville" },
      });
    });
  ...
  describe("Props Functionality", () => {
    it("uses the specified `submitComponent`", async () => {
      const DistinctSubmitButton = defineComponent({
        template:
          '<button type="submit" class="distinct-submit-button">Distinct Submit</button>',
        name: "DistinctSubmitButton",
      });
      const wrapper = createFormWrapper(
        {
          fields: [
            { propertyName: "test", component: "StubAppInput", value: "" },
          ],
          submitComponent: "DistinctSubmitButton",
        },
        { global: { components: { DistinctSubmitButton } } }
      );
      await nextTick();
    ...
    it("applies `submitBtnClasses` to the submit component", async () => {
      ...
      await nextTick();
      expect(wrapper.findComponent(StubAppSubmit).classes()).toContain(
        "extra-class-1"
      );
    });
    ...
    it("passes `submitBtnProps` to the submit component", async () => {
      const wrapper = createFormWrapper({
        submitBtnProps: { text: "Go", disabled: true },
      });
      await nextTick();
    ...
    describe("errorProps functionality", () => {
      const CustomErrorPropInput = defineComponent({
        ...
        template:
          '<div><input :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" /><div v-if="isInvalid && validationMsg" class="custom-error-display">{{ validationMsg }}</div></div>',
        props: ["modelValue", "label", "isInvalid", "validationMsg"],
      ...
      it("passes custom error prop names to field items when `errorProps` is configured", async () => {
        ...
        const wrapper = createFormWrapper(
          {
            fields: [
              {
                propertyName: "field1",
                component: "CustomErrorPropInput",
                rules: ["required"],
                value: "",
              },
            ],
          ...
          { global: { components: { CustomErrorPropInput } } }
        );
        await nextTick();
        await nextTick();
        await wrapper.find("form").trigger("submit.prevent");
        ...
        expect(inputComponent.props("validationMsg")).toBe(
      ...
      it('passes error messages as string even if errorMessagesType is "array"', async () => {
        ...
        const wrapper = createFormWrapper({
          ...
          modelValue: { field1: "" },
          errorProps: customErrorPropsArray,
        });
        await nextTick();
        await nextTick();
        await wrapper.find("form").trigger("submit.prevent");
  ...
  describe("Slots Functionality", () => {
    it('renders content in the "title" slot, overriding the title prop', async () => {
      const wrapper = createFormWrapper(
        { title: "PropTitle" },
        { slots: { title: `<h1 class="slotted-title">SlotTitle</h1>` } }
      );
      await nextTick();
    ...
    it('renders content in the "submit-row" slot, replacing default submit button', async () => {
      const wrapper = createFormWrapper(
        ...
        {
          slots: {
            "submit-row": `<div class="custom-submit-area"><button>SlotSubmit</button></div>`,
          },
        }
      );
      await nextTick();
    ...
    it("renders content in the default slot at the end of the form", async () => {
      const wrapper = createFormWrapper(
        {
          fields: [
            { propertyName: "name", component: "StubAppInput", value: "test" },
          ],
        },
        {
          slots: { default: `<p class="default-slot-content">DefaultSlot</p>` },
        }
      );
      await nextTick();
      await nextTick();
  ...
  describe("Field and Form Interaction States (Touched/Dirty)", () => {
    ...
    it("marks field as touched on blur and emits @field:touched", async () => {
      const nameInputWrapper = wrapper
        .findAllComponents(StubAppInput)
      ...
      expect(touchedEvents[0][0]).toEqual({
        propertyName: "name",
        touched: true,
      });
    });
    ...
    it("marks field as dirty on value change and emits @field:dirty", async () => {
      ...
      await nextTick();
      await wrapper.setProps({
        modelValue: { ...wrapper.props("modelValue"), name: "NewName" },
      });
      await nextTick();
      ...
      expect(dirtyEvents[dirtyEvents.length - 1][0]).toEqual({
        propertyName: "name",
        dirty: true,
      });
    });
    ...
    it("marks field as not dirty if value reverts to initial and emits @field:dirty", async () => {
      ...
      await wrapper.setProps({
        modelValue: { ...wrapper.props("modelValue"), name: "InitialName" },
      });
      await nextTick();
    ...
    it("exposes isFormDirty and isFormTouched via default scoped slot", async () => {
      ...
      const slotWrapper = mount(PreskoForm, {
        ...
        slots: {
          "default-extra": (slotProps) => h(TestSlotComponent, { slotProps }),
        },
      });
      await nextTick();
      await nextTick();

      expect(slotWrapper.find('[data-testid="isFormDirtyInSlot"]').text()).toBe(
        "false"
      );
      expect(
        slotWrapper.find('[data-testid="isFormTouchedInSlot"]').text()
      ).toBe("false");

      const nameInputWrapper = slotWrapper
        .findAllComponents(StubAppInput)
        .find((c) => c.props("label") === "Name");
      await nameInputWrapper.vm.$emit("blur");
      nameInputWrapper.vm.$emit("update:modelValue", "NewName");
      await slotWrapper.setProps({
        modelValue: { ...initialModel, name: "NewName" },
      });
      await nextTick();
      await nextTick();
