...
export function useFormValidation(fields) {
  ...
  const getFieldLabel = (field) => {
    return field.label || (field.props && field.props.label) || field.propertyName;
  };
    ...
    if (fields && Array.isArray(fields)) {
      fields.forEach(field => {
        if (field.propertyName) {
          ...
          initialValuesTemp[field.propertyName] = initialValue !== undefined ? JSON.parse(JSON.stringify(initialValue)) : undefined;
    ...
    for (const rule of field.rules) {
      ...
      if (typeof rule === "string") {
        ...
        if (name === 'matchRegex' && regex instanceof RegExp) {
          validity = Validation.matchRegex(input, labelArg, customErrorMsg, regex);
  ...
  const validateField = (field, input) => {
    const fieldConfig = typeof field === 'string' ? fields.find(f => f.propertyName === field) : field;
    ...
    if (fields && Array.isArray(fields)) {
      for (const propertyName in formToValidate) {
        if (formToValidate.hasOwnProperty(propertyName)) {
          const fieldConfig = fields.find(f => f.propertyName === propertyName);
          if (fieldConfig) {
  ...
  const checkFieldDirty = (fieldName, currentValue) => {
    if (initialFormFieldsValues.hasOwnProperty(fieldName)) {
      ...
      const newIsDirty = JSON.stringify(currentValue) !== JSON.stringify(initialFormFieldsValues[fieldName]);
